...
public class Auto_Blue_Left extends LinearOpMode {
     ...
     */
    private void setSliderPosition(double sliderPosition, int sleepTime) {
    ...
    private void main() {
        ...
        //Move fwd abd get cone
        setGripperState(true);
        setSliderPosition(13,250);
        setSliderPosition(13,250);
        moveDistance(4, 1,8,true,0);

        //lift cone and rotate to next post
        rotateAngleGlobal(-42, 1, 1);

        //move fwd to post and release cone
        moveDistance(7, 1,8,true,-42);
        setSliderPosition(11,300);
        setGripperState(false);

        //move aft and open gripper lower slider
        moveDistance(-7, 1,8,true,-42);
        setSliderPosition(0,0);
        setGripperState(false);

        //rotate to zero
        rotateAngleGlobal(0,1,1);

        //move long distance and move back and raise slider
        moveDistance(44,4,8,true,0);
        setSliderPosition(5,0);
        moveDistance(-13, 1,8,true,0);


        //open gripper and move while rotating, move back an inch
        setGripperState(false);
        moveDistance(35,  0.9, 8, true, 90);
        //moveDistance(-1,  1, 8, true, -90);

        //close gripper and raise
        setGripperState(true);
        setSliderPosition(13,300);

        //move back, rotate, move fwd, open gripper, move back, lower slider to second cone
        moveDistance(-5, 1, 8, true, 90);
        rotateAngleGlobal(-150, 1, 1);
        moveDistance(3, 1, 8, true, -150);
        setSliderPosition(11,0);
        setGripperState(false);
        moveDistance(-3, 1, 8, true, -150);
        setSliderPosition(4,0);

        //rotate towards tower, move fwd, move back, rotate move fwd, release, move back
        setGripperState(false);
        rotateAngleGlobal(90, 1, 1);
        moveDistance(4, 1, 8, true, 90);
        setGripperState(true);
        setSliderPosition(13,250);
        moveDistance(-4, 1, 8, true, 90);
        rotateAngleGlobal(-150, 1, 1);
        ...
        rotateAngleGlobal(-90, 1, 1);
        if (getParkingSpot==3) {
            moveDistance(40, 1, 8, true, -90);
        } else if (getParkingSpot == 2) {
            moveDistance(18, 1, 8, true, -90);
        } else if (getParkingSpot == 1) {
            moveDistance(-6, 1, 8, true, -90);
        }
    }
